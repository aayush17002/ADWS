 ...
 */
#define MAX_HCLIB_ASYNC_ARG_SIZE 16
...
struct hclib_async_task  {
	char _args[MAX_HCLIB_ASYNC_ARG_SIZE];
	...
	inline void init_hclib_async_task(generic_framePtr fp, size_t arg_sz, void *async_args) {
		HASSERT(arg_sz <= MAX_HCLIB_ASYNC_ARG_SIZE);
		this->_fp = fp;
	...
	inline hclib_async_task() { };

	template<typename Function, typename T1>
	inline hclib_async_task(Function kernel, const T1 &a1) {
		async_arguments1<Function, T1> args(kernel, a1);
		init_hclib_async_task(wrapper1<Function, T1>, (size_t)sizeof(args), (void *)&args);
	}
};

#define task_t 	hclib_async_task
